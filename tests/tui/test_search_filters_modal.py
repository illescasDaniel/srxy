from __future__ import annotations

import asyncio

import pytest
from tests.tui.helpers import assert_svg_snapshot
from textual.app import App, ComposeResult
from textual.widgets import Static

from srxy.cli import build_parser
from srxy.tui.app import SrxyApp
from srxy.tui.modals import SearchFiltersModal
from srxy.tui.search_filters import SearchFilters
from srxy.tui.size_limits import SizeLimits


pytestmark = [pytest.mark.integration, pytest.mark.tui]


class _ModalHostApp(App[None]):
	def compose(self) -> ComposeResult:
		yield Static("host", id="host")


@pytest.mark.parametrize("theme", ["textual-light", "textual-dark"])
def test_given_search_filters_modal_when_typing_in_field_then_matches_snapshot(theme: str):
	# given
	app = _ModalHostApp()
	app.theme = theme
	initial = SearchFilters(
		top_files="",
		max_matches="50",
		size_limits=SizeLimits(text_mib="100", ocr_mib="50", transcribe_mib="500"),
		threshold="35",
		semantic_image_threshold="18",
		transcribe_threshold="25",
	)

	async def run():
		async with app.run_test(size=(80, 28)) as pilot:
			app.push_screen(SearchFiltersModal(initial), wait_for_dismiss=False)
			await pilot.pause()
			await pilot.click("#sf-top-files")
			await pilot.pause()
			await pilot.press("2", "5")
			await pilot.pause()
			assert isinstance(app.screen, SearchFiltersModal)
			svg = app.export_screenshot(title="search-filters-modal-typing")
			assert_svg_snapshot(f"search_filters_modal_typing_{theme}", svg)

	asyncio.run(run())


@pytest.mark.parametrize("theme", ["textual-light", "textual-dark"])
def test_given_tui_when_typing_in_filters_dialog_then_matches_snapshot(theme: str):
	# given
	args = build_parser().parse_args(["hello", "."])
	app = SrxyApp(args, auto_start=False)
	app.theme = theme

	async def run():
		async with app.run_test(size=(100, 40)) as pilot:
			await pilot.pause()
			await pilot.click("#search-filters-button")
			await pilot.pause()
			assert isinstance(app.screen, SearchFiltersModal)
			await pilot.click("#sf-max-matches")
			await pilot.pause()
			await pilot.press("1", "0", "0")
			await pilot.pause()
			svg = app.export_screenshot(title="srxy-tui-filters-typing")
			assert_svg_snapshot(f"srxy_app_filters_typing_{theme}", svg)

	asyncio.run(run())
