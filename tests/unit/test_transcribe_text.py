...
def test_given_mocked_transcription_when_iterating_lines_then_yields_segments(
	...
	with (
		...
		patch(
			"srxy.adapters.outbound.transcribe.transcribe_text._transcribe_wav_segments",
			side_effect=fake_transcribe_wav_segments,
		),
		patch(
			"srxy.adapters.outbound.transcribe.transcribe_text._cached_transcript_lines",
			side_effect=lambda _hash, fn: fn()[1],
		),
	):
		# when
...
def test_given_ffmpeg_on_path_when_checking_available_then_uses_which(monkeypatch: pytest.MonkeyPatch):
	# given
	monkeypatch.setattr(
		"srxy.adapters.outbound.transcribe.transcribe_text.shutil.which",
...
def test_given_transcribe_threshold_env_when_reading_then_returns_float(monkeypatch: pytest.MonkeyPatch):
	...
	# when / then
	from srxy.adapters.outbound.transcribe.transcribe_text import transcribe_threshold_value
...
def test_given_transcribe_deps_installed_when_checking_then_uses_find_spec(monkeypatch: pytest.MonkeyPatch):
	...
	monkeypatch.setattr("srxy.adapters.outbound.transcribe.transcribe_text.importlib.util.find_spec", fake_find_spec)
...
def test_given_cached_transcript_when_iterating_twice_then_transcribes_once(
	...
	with (
		...
		patch(
			"srxy.adapters.outbound.transcribe.transcribe_text._transcribe_wav_segments", side_effect=fake_transcribe
		) as transcribe_mock,
	):
		# when
...
def test_given_empty_transcript_when_caching_then_does_not_store_empty_payload(
	...
	from srxy.adapters.outbound.cache.cache import CACHE_KIND_TRANSCRIPT, cache_get, get_file_content_hash
...
def test_given_cuda_faster_whisper_failure_when_transcribing_then_falls_back_to_transformers(
	...
	with (
		...
		patch("srxy.adapters.outbound.transcribe.transcribe_text._iter_faster_whisper_segment_lines", side_effect=boom),
		patch(
			"srxy.adapters.outbound.transcribe.transcribe_text._iter_transformers_segment_lines",
			side_effect=transformers,
		),
		patch(
		...
		from srxy.adapters.outbound.transcribe.transcribe_text import (
			_transcribe_wav_segments,  # pyright: ignore[reportPrivateUsage]
		)
...
def test_given_cpu_faster_whisper_empty_when_transcribing_then_falls_back_to_transformers(
	...
	with (
		patch(
			"srxy.adapters.outbound.transcribe.transcribe_text._iter_faster_whisper_segment_lines", side_effect=empty
		),
		patch(
			"srxy.adapters.outbound.transcribe.transcribe_text._iter_transformers_segment_lines",
			side_effect=transformers,
...
def test_given_seconds_when_formatting_transcript_timestamp_then_uses_mm_ss():
	# when / then
	from srxy.adapters.outbound.transcribe.transcribe_text import format_transcript_timestamp
...
def test_given_segment_text_when_building_segment_then_omits_timestamp_from_text():
	# given
	from srxy.adapters.outbound.transcribe.transcribe_text import _segment_at  # pyright: ignore[reportPrivateUsage]
...
def test_given_transcript_cache_payload_when_round_tripping_then_preserves_timestamp_and_text(
	...
	from srxy.adapters.outbound.transcribe.transcribe_text import (
		_cached_transcript_lines,  # pyright: ignore[reportPrivateUsage]
	)
...
def test_given_nvidia_cublas_lib_when_ensuring_cuda_libs_then_preloads_library(
	...
	import srxy.adapters.outbound.transcribe.transcribe_text as module
