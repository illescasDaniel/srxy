...
from srxy.domain.text import collapse_whitespace, normalize_text, query_words


PreviewHighlight = Literal["guillemets", "bold", "html", "none"]
...
def find_match_span(text: str, query: str, *, highlight_term: str | None = None) -> tuple[int, int]:
	...
	from srxy.domain.file_query import query_highlight_terms
...
def _find_semantic_word_span(text: str, query: str) -> tuple[int, int] | None:
	from srxy.application.matching.registry import get_atomic_matcher, is_matcher_available
	from srxy.domain.models import MatchType
...
def _wrap_match_highlight(match_text: str, *, highlight: PreviewHighlight) -> str:
	...
	if highlight == "html":
		return f"<b>{html.escape(match_text)}</b>"
...
def format_match_preview(
	...
	*,
	...
	if highlight_term:
		...
		from srxy.domain.file_query import parse_file_query, query_highlight_terms, query_is_compound
