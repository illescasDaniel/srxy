...
.DESCRIPTION
  On Windows, `uv sync --extra semantic` installs the CPU-only torch wheel from PyPI.
  That silently makes semantic/CLIP/transcribe tests run on CPU. This script detects
  that mismatch and reinstalls torch/torchvision/torchaudio from the PyTorch CUDA index.
...
if (-not (Test-Path -LiteralPath $python)) {
	Write-Host "ensure-windows-cuda-torch: missing $python (run uv sync --extra semantic --extra windows first)" -ForegroundColor Yellow
	exit 0
}
