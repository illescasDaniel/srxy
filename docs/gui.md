...
| **How to search** | Options and Filters buttons (stacked) open popup dialogs; each control has an **(i)** info button |
| **Search** | Wider Search button (enabled only when path + query are usable); warning icon when the query is invalid; stale orange highlight when settings changed |
...
| **Search progress** | Progress bar, percentage (stays at 100% when done), status, Cancel; inactive until the first search |
