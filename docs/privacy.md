...
The optional **desktop installer** (Linux AppImages and macOS `.app` / DMG wrappers) does **not** ship NVIDIA/CUDA binaries, Hugging Face model weights, Tesseract, or ffmpeg inside the installer artifact. When you opt in, it downloads components from third parties. Review their sites and privacy policies before continuing. See [installers.md](installers.md).
