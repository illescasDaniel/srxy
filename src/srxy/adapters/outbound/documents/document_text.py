...
def iter_document_metadata_lines(path: Path) -> Iterator[tuple[int, str]]:
	...
	if sys.platform == "win32":
		from srxy.adapters.outbound.metadata.windows_metadata import windows_metadata_supported
...
def _document_ocr_active(path: Path, ocr: bool | None) -> bool:
	from srxy.adapters.outbound.ocr.ocr_text import is_ocr_active, ocr_max_file_size
...
def iter_document_lines(
	...
	*,
	...
	from srxy.adapters.outbound.cache.cache import CACHE_KIND_DOCUMENT_TEXT, cache_get, cache_put, get_file_content_hash
...
def _iter_pdf_lines(
	...
	*,
	...
	from srxy.adapters.outbound.ocr.ocr_text import ocr_pdf_page_images
...
def _iter_docx_lines(
	...
	*,
	...
	from srxy.adapters.outbound.ocr.ocr_text import ocr_image_bytes
...
def _iter_pptx_lines(
	...
	*,
	...
	from srxy.adapters.outbound.ocr.ocr_text import MIN_PDF_IMAGE_OCR_BYTES, ocr_image_bytes
