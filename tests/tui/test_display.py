...
_FIELD_LABELS = ("Query", "Path", "Search options", "Filters")
...
def test_given_options_summary_when_composed_then_shows_enabled_modes():
	# given
...
def test_given_tui_when_composed_then_search_row_has_padding_and_controls_are_slightly_taller():
	...
	async def run():
		async with app.run_test(size=(100, 30)) as pilot:
			...
			svg = app.export_screenshot(title="srxy-tui")
			assert_labels_visible(svg, ("Search", "Search options", "Filters"))
